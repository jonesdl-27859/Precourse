...
function hasEmail(user) {
  ...
  // otherwise return false
    if(Object.hasOwnProperty.call(user, 'email') && Boolean(user['email'])) {
      return true;
    }else{
      return false;
    }
...
function setUsersToPremium(users) {
  ...
  for(let i = 0; i < users.length; i++){//loop through array
  users[i].isPremium = true;//set isPremium to true
 }
 return users;
}
